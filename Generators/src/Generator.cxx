...
namespace eventgen
{
  ...
  Generator::~Generator()
  {
    ...
    for (auto &trigger : mTriggers) delete trigger;
  ...
  Generator::ReadEvent(FairPrimaryGenerator *primGen)
  {
	...
	return kFALSE;
      ...
      /** boost event **/
      if (!boostEvent(mBoost)) return kFALSE;      
  ...
  Generator::triggerEvent() const
  {
    ...
    if (mTriggers.size() == 0) return kTRUE;
    else if (mTriggerMode == kTriggerOFF) return kTRUE;
    else if (mTriggerMode == kTriggerOR) triggered = kFALSE;
    else if (mTriggerMode == kTriggerAND) triggered = kTRUE;
    else return kTRUE;
    
    /** loop over triggers **/
    for (const auto &trigger : mTriggers) {
      auto retval = triggerFired(trigger);
      if (mTriggerMode == kTriggerOR) triggered |= retval;
      if (mTriggerMode == kTriggerAND) triggered &= retval;
